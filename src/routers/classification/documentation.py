...
from utils.custom_error import AccessUnauthorized, ServiceError, DatabaseQueryError, DiVA
...
async def labels_documentation(request: Request, db: Annotated[AsyncSession, Depends(get_db)]) -> ResponseDefault:
    ...
    object_record = await find_record(db=db, table=ObjectDocumentationDetails, fetch_type="all")
    env_record = await find_record(db=db, table=EnvironmentDocumentationDetails, fetch_type="all")
    design_type_record = await find_record(db=db, table=DesignTypeDocumentationDetails, fetch_type="all")
    time_period_record = await find_record(db=db, table=TimePeriodDocumentationDetails, fetch_type="all")
    dominant_color_record = await find_record(db=db, table=DominantColorDocumentationDetails, fetch_type="all")
    culture_style_record = await find_record(db=db, table=CultureStyleDocumentationDetails, fetch_type="all")
