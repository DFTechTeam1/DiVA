...
async def test_move_nas_multi_dir_with_mix_existing_and_non_existing_nas_dir() -> None:
    ...
    payload = {
        ...
        "target_folder": [
            existing_dir[
                randint(0, len(await list_folder(ip_address=config.NAS_IP_5, directory_path="/nas_testing/api_testing")) - 1)
            ]["path"],
            existing_dir[
                randint(0, len(await list_folder(ip_address=config.NAS_IP_5, directory_path="/nas_testing/api_testing")) - 1)
            ]["path"],
...
async def test_move_nas_non_existing_directory_on_nas() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        response = res.json()
        assert res.status_code == 200
        assert response["message"] == "Target folder should be existing directory on NAS."
...
async def test_move_nas_non_with_invalid_destination_folder_path_params() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        response = res.json()
        assert res.status_code == 200
        assert response["message"] == "Destination folder should be existing directory on NAS."
        assert response["data"]["folder_already_exsist"] is None
        assert len(response["data"]["non_existing_folder"]) == 1


@pytest.mark.asyncio
async def test_move_nas_non_with_invalid_length_of_target_folder_and_destination_path() -> None:
    """Should raise error due to length of target folder and destination folder path should be equal"""
    payload = {
        "ip_address": config.NAS_IP_5,
        "target_folder": ["/nas_testing/api_testing/tetsing1", "/nas_testing/api_testing/tetsing1"],
        "dest_folder_path": [destination],
    }
