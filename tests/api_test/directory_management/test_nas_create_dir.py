...
async def test_create_nas_single_dir_with_valid_payload_using_string() -> None:
    ...
    payload = {"ip_address": config.NAS_IP_5, "shared_folder": "/nas_testing", "target_folder": f"api_testing/{str(uuid4())}"}
...
async def test_create_nas_multi_dir_with_valid_payload() -> None:
    ...
    payload = {
        ...
        "target_folder": [f"api_testing/{str(uuid4())}", f"api_testing/{str(uuid4())}", f"api_testing/{str(uuid4())}"],
    }
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/create-dir", json=payload)
        response = res.json()
...
async def test_create_nas_single_directory_already_exist_in_nas():
    ...
    payload = {"ip_address": config.NAS_IP_5, "shared_folder": "/nas_testing", "target_folder": "existing_dir/already_exist1"}
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/create-dir", json=payload)
        response = res.json()
        assert res.status_code == 200
...
async def test_create_nas_multi_directory_all_already_exist_in_nas():
    ...
    payload = {
        "ip_address": config.NAS_IP_5,
        "shared_folder": ["/nas_testing", "/nas_testing", "/nas_testing"],
        "target_folder": ["existing_dir/already_exist1", "existing_dir/already_exist2", "existing_dir/already_exist3"],
    }
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/create-dir", json=payload)
        response = res.json()
...
async def test_create_nas_multi_directory_mix_of_new_and_dir_already_exist_on_nas():
    ...
    payload = {
        "ip_address": config.NAS_IP_5,
        "shared_folder": ["/nas_testing", "/nas_testing", "/nas_testing"],
        "target_folder": [f"api_testing/{str(uuid4())}", "existing_dir/already_exist1", "existing_dir/already_exist2"],
    }
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/create-dir", json=payload)
        response = res.json()
