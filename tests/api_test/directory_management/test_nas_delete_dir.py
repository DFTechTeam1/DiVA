...
async def test_delete_nas_multi_dir_with_valid_payload_using_array() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        assert res.status_code == 200
        assert response["message"] == "Directory successfully removed."
        assert len(response["data"]["folder_already_exsist"]) == len(payload["target_folder"])
        assert response["data"]["non_existing_folder"] is None
...
async def test_delete_nas_multi_dir_with_non_existing_folder_on_nas() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        assert len(response["data"]["non_existing_folder"]) == len(payload["target_folder"])
        assert response["data"]["folder_already_exsist"] is None


@pytest.mark.asyncio
async def test_delete_nas_multi_dir_with_directory_already_exist_and_new_directory_input() -> None:
    """Should remove a multi directory"""
    existing_dir = await list_folder(ip_address=config.NAS_IP_5, directory_path="/nas_testing/api_testing")
    random_index = randint(0, len(existing_dir) - 1)
...
@pytest.mark.asyncio
async def test_delete_nas_single_dir_with_invalid_shared_folder_params_using_array() -> None:
    """Should skip deletion directory due to folder not found"""
    payload = {"ip_address": config.NAS_IP_5, "target_folder": ["random_shared_folder/also_random_dir"]}
...
@pytest.mark.asyncio
async def test_delete_nas_single_dir_with_invalid_shared_folder_params_using_string() -> None:
    """Should skip deletion directory due to folder not found"""
    payload = {"ip_address": config.NAS_IP_5, "target_folder": "random_shared_folder/also_random_dir"}
...
@pytest.mark.asyncio
async def test_delete_nas_multi_dir_with_invalid_shared_folder_params_using_array() -> None:
    """Should skip deletion directory due to folder not found"""
    payload = {
        "ip_address": config.NAS_IP_5,
        "target_folder": ["random_shared_folder/also_random_dir1", "random_shared_folder/also_random_dir2"],
    }
