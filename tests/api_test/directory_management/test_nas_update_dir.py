...
async def test_update_nas_single_dir_with_valid_payload_using_string() -> None:
    ...
    payload = {"ip_address": config.NAS_IP_5, "target_folder": existing_dir[0]["path"], "changed_name_into": f"{str(uuid4())}"}
...
async def test_update_nas_single_dir_with_valid_payload_using_array() -> None:
    ...
    payload = {"ip_address": config.NAS_IP_5, "target_folder": [random_path], "changed_name_into": [f"{str(uuid4())}"]}
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/update-dir", json=payload)
        response = res.json()
        assert res.status_code == 200
...
async def test_update_nas_multi_dir_with_valid_payload_using_array() -> None:
    ...
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        assert res.status_code == 200
        assert response["message"] == "Directory renamed successfully."
        assert len(response["data"]["non_existing_folder"]) == len(payload["changed_name_into"])
        assert response["data"]["folder_already_exsist"] is None
...
async def test_update_nas_multi_dir_with_mix_directory_using_array() -> None:
    ...
    payload = {
        "ip_address": config.NAS_IP_5,
        "target_folder": [existing_dir[0]["path"], existing_dir[1]["path"], existing_dir[2]["path"]],
        "changed_name_into": [f"{str(uuid4())}", f"{str(uuid4())}", "already_exist_dir1"],
    }
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/update-dir", json=payload)
        response = res.json()
...
@pytest.mark.asyncio
async def test_update_nas_single_dir_with_change_name_into_directory_already_exist_using_string() -> None:
    """Should skipping update directory due to changed_name_into and matched into existing dir on NAS"""

    existing_dir = await list_folder(ip_address=config.NAS_IP_5, directory_path="/nas_testing/api_testing")

    payload = {"ip_address": config.NAS_IP_5, "target_folder": existing_dir[0]["path"], "changed_name_into": "already_exist_dir1"}
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        res = await client.post("/api/v1/nas/update-dir", json=payload)
        response = res.json()
        assert res.status_code == 200
